...
bool isPlainTextValid(string plain_text)
{
    ...
    for (int i = 0; i < plain_text.size(); i++)
    {
        if (!((plain_text[i] >= '0' && plain_text[i] <= '9') ||
              (plain_text[i] >= 'A' && plain_text[i] <= 'Z') || (plain_text[i] >= 'a' && plain_text[i] <= 'z')))
        {
            return false;
        }
    }
...
int main()
{
    ...
    {
        cout << "Please enter the valid plain text in hexadecimal.";
        return 0;
    }
    ...
    cout << "Decrypted text: " << decryption(encrypted_text, round_key);
    return 0;
}
